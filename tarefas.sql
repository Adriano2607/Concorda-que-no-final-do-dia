...
select idProprietario,categoria,nome as NomeCompleto from proprietario 
...
select ano,Descrição,motor as Motor from carro 
...
SELECT HEX(lucro) as 'representacao Hexadecimal ' from custo where idCusto = 5;
SELECT AVG(Ano) AS 'Media Ano' FROM carro;
SELECT MAX(Valor_de_custo) 'Maximo valor de custo' FROM custo;
SELECT MIN(lucro) 'Minimo lucro' FROM custo;
SELECT COUNT(idVendas) as 'numero de vendas' FROM vendas;
SELECT (Valor_de_Venda - Valor_de_custo) as 'lucro' from custo where idCusto = 1; -- esse
...
CREATE VIEW visao_chassi AS SELECT * FROM chassi;
...
insert into placa values (10,'ERD-0145','00523042007-B','Mercosul');
savepoint atalho5;
...
grant select on adrianonovo.visao_chassi to 'teste1'@'localhost';
grant insert on adrianonovo.carro to 'teste2'@'localhost';
grant select on adrianonovo.placa to 'teste3'@'localhost';
